	...
	for e in range(args.epochs):
		with benchmark_total:
			...
			with benchmark_test:
				...
				precision = [tm.clause_precision(i, True, X_train, number_of_examples=500) for i in tqdm(range(len(target_words)), desc="Precision")]

				_LOGGER.info("Calculating recall")
				recall = [tm.clause_recall(i, True, X_train, number_of_examples=500) for i in tqdm(range(len(target_words)), desc="Recall")]

				_LOGGER.info("Clauses")
				for j in range(args.clauses):
					clause_info = " ".join([f"{target_words[i]}:W{tm.get_weight(i, j)}:P{precision[i][j]:.2f}:R{recall[i][j]:.2f}" for i in range(len(target_words))])
					literals = ["{}{}({})".format("¬" if k >= tm.clause_bank.number_of_features else "", feature_names[k % tm.clause_bank.number_of_features], tm.clause_bank.get_ta_state(j, k)) for k in range(tm.clause_bank.number_of_literals) if tm.get_ta_action(j, k) == 1]
					_LOGGER.info(f"Clause #{j} {clause_info} {' ∧ '.join(literals)}")

				profile = np.array([np.where(tm.get_weights(i) >= args.clause_weight_threshold, tm.get_weights(i), 0) for i in range(len(target_words))])
				similarity = cosine_similarity(profile)

				_LOGGER.info("Word Similarity:")
				for i in range(len(target_words)):
					sorted_index = np.argsort(-similarity[i, :])
					similarity_info = " ".join([f"{target_words[sorted_index[j]]}({similarity[i, sorted_index[j]]:.2f})" for j in range(1, len(target_words))])
