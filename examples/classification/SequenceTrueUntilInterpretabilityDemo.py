...
def main(args):
    ...
    Y_train = np.random.randint(0, 2, size=(args.examples), dtype=np.uint32)
    ...
    Y_test = np.random.randint(0, 2, size=(args.examples//10), dtype=np.uint32)
    for i in range(args.examples//10):
        ...
        if Y_test[i] == 0:
            ...
            X_test[i,0,position_4,0] = 0
            X_test[i,0,position_4,1] = 1

            X_test[i,0,position_5,0] = 0
            X_test[i,0,position_5,1] = 0

    tm = TMClassifier(args.number_of_clauses, args.T, args.s, number_of_state_bits_ta=10, patch_dim=(1, 1), weighted_clauses=False, platform=args.platform, boost_true_positive_feedback=True, spatio_temporal=True, incremental=False, max_included_literals=args.max_included_literals)
    ...
    for i in range(args.epochs):
        ...
        for j in range(args.number_of_clauses // 2):
            ...
            for k in range(tm.clause_banks[0].number_of_features):
                ...
                if k in range(tm.clause_banks[0].number_of_clauses*4, tm.clause_banks[0].number_of_clauses*5):
                    if tm.get_ta_action(j, k, the_class=the_class, polarity=polarity):
